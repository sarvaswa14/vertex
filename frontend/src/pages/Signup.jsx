    ...
    <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#080c14' }}>
      <div style={{ width: '100%', maxWidth: '400px', padding: '0 1.5rem' }}>
        ...
        <div style={{ background: 'rgba(15,20,35,0.8)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: '16px', padding: '2rem', boxShadow: '0 0 40px rgba(59,130,246,0.05), 0 20px 60px rgba(0,0,0,0.4)' }}>
          <h2 style={{ color: '#f0f4ff', fontSize: '1.1rem', fontWeight: 600, marginBottom: '0.25rem' }}>Create account</h2>
          <p style={{ color: '#4a5568', fontSize: '0.8rem', marginBottom: '1.5rem' }}>Start your productivity journey</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            {[
              ...
              <div key={field.label}>
                <label style={{ color: '#6b7280', fontSize: '0.75rem', fontWeight: 500, display: 'block', marginBottom: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{field.label}</label>
                <input type={field.type} value={field.value} onChange={e => field.setter(e.target.value)} placeholder={field.placeholder} required={field.type !== 'text'}
